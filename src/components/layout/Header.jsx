...
const Header = ({ onToggleSidebar, isSidebarOpen, isMobile }) => {
  ...
  return (
    <header className="bg-white/95 backdrop-blur-xl border-b border-gray-200/80 sticky top-0 z-40 shadow-sm shadow-black/5">
      <div className="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
        {/* Lado izquierdo - Navegación móvil */}
        <div className="flex items-center">
          {/* Botón menú móvil - AGREGADO data-sidebar-toggle */}
          <button
            onClick={onToggleSidebar}
            data-sidebar-toggle="true" // IMPORTANTE: Para identificar el botón toggle
            className={`p-2.5 text-gray-600 hover:text-gray-900 hover:bg-gray-100/80 rounded-xl transition-all duration-300 ease-out focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:ring-offset-2 ${
              isMobile ? 'lg:hidden' : 'hidden'
            }`}
          ...
          <div className={`flex items-center space-x-3 ${isMobile ? 'lg:hidden' : 'hidden'}`}>
            ...
            <div className="flex flex-col">
              <h1 className="text-lg font-bold text-gray-900 tracking-tight">Recorridos</h1>
              <p className="text-xs text-gray-600 font-medium">Panel de control</p>
            </div>
          </div>
        </div>

        {/* Resto del código del Header permanece igual */}
        <div className="flex items-center space-x-4">
          ...
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className="group flex items-center space-x-3 p-2 rounded-2xl hover:bg-gray-50/80 transition-all duration-300 ease-out border border-transparent hover:border-gray-200/50 focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:ring-offset-2"
            >
              ...
              <div className="flex items-center space-x-2.5">
                ...
                <ChevronDown 
                  size={16} 
                  className={`text-gray-500 transition-all duration-300 ease-out ${
                    showUserMenu ? 'rotate-180 text-gray-700' : 'group-hover:text-gray-700'
                  }`} 
                />
              </div>
            </button>
            ...
            {/* Dropdown menu */}
            {showUserMenu && (
              <div className="absolute right-0 top-full mt-2 w-72 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200/80 py-2 z-50 ring-1 ring-black/5 animate-in fade-in-0 zoom-in-95">
                {/* Header del menú */}
                ...
                <div className="py-2">
                  <button
                    ...
                    className="flex items-center space-x-3 w-full px-5 py-3 text-sm text-gray-700 hover:bg-gray-50/80 transition-all duration-200 ease-out group"
                  >
                  ...
                  <div className="mx-5 my-1 h-px bg-gray-200/60"></div>

                  <button
                    onClick={handleLogout}
                    className="flex items-center space-x-3 w-full px-5 py-3 text-sm text-red-600 hover:bg-red-50/80 transition-all duration-200 ease-out group"
                  >
                ...
                {/* Footer del menú */}
                <div className="px-5 py-3 border-t border-gray-200/60 bg-gray-50/50 rounded-b-2xl">
